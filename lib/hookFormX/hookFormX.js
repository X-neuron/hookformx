...
function useFormInput(name, validateStyle, defaultValues, validateSchema, values, handleError, isTouched, errors) {
  ...
  validateStyle = capitalize(validateStyle); // check whether need to export check property
  ...
  return _ref = {
    value: value
  }, _defineProperty(_ref, "check".concat(isChecked.current), value), _defineProperty(_ref, "required", defSchema.current.required), _defineProperty(_ref, "defaultValue", defaultValues[name]), _defineProperty(_ref, "onFocus", handleFocus), _defineProperty(_ref, "error", isTouched.current[name] && error.hasError), _defineProperty(_ref, "helperText", isTouched.current[name] ? error.errorMessage : ''), _defineProperty(_ref, "on".concat(validateStyle), validateStyle === 'Change' ? handleChange : handleBlur), _ref;
}